...
import com.example.blockchaindemo.webSocket.P2PClient;
import com.example.blockchaindemo.webSocket.P2PServer;
...
@Service
public class P2PServiceImpl implements P2PService, ApplicationRunner {
    ...
    public void handleMessage(WebSocket webSocket, String msg, List<WebSocket> sockets) {
        try {
            ...
            switch (message.getType()) {
                //客户端请求查询最新的区块:1
                case BlockConstant.QUERY_LATEST_BLOCK -> write(webSocket, responseLatestBlockMsg());//服务端调用方法返回最新区块:2

                //接收到服务端返回的最新区块:2
                case BlockConstant.RESPONSE_LATEST_BLOCK -> handleBlockResponse(message.getData(), sockets);

                //客户端请求查询整个区块链:3
                case BlockConstant.QUERY_BLOCKCHAIN -> write(webSocket, responseBlockChainMsg());//服务端调用方法返回最新区块:4

                //直接接收到其他节点发送的整条区块链信息:4
                case BlockConstant.RESPONSE_BLOCKCHAIN -> handleBlockChainResponse(message.getData(), sockets);
            }
        } catch (Exception e) {
